...
public class activity_Arroz_Agulha_amanhecer extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        contactList.add(new Contact("Arroz Agulha AMANCHECER", "1,85€", R.drawable.amahecer_arroz_agulha));
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    if (snapshot.exists()) {
                        ...
                        if (nomeUsuario != null && !nomeUsuario.isEmpty()) {
                            Intent intent = new Intent(activity_Arroz_Agulha_amanhecer.this, CarrinhoActivity.class);
                            intent.putExtra("nome", nomeUsuario);
                            startActivity(intent);
                        } else {
                            Toast.makeText(activity_Arroz_Agulha_amanhecer.this, "Nome do cliente não encontrado.", Toast.LENGTH_SHORT).show();
                        }
                    } else {
                        Toast.makeText(activity_Arroz_Agulha_amanhecer.this, "Cliente não encontrado.", Toast.LENGTH_SHORT).show();
                    }
                }

                @Override
                public void onCancelled(@NonNull DatabaseError error) {
                    Toast.makeText(activity_Arroz_Agulha_amanhecer.this, "Erro ao recuperar dados do cliente.", Toast.LENGTH_SHORT).show();
                }
            });
        } else {
            Toast.makeText(activity_Arroz_Agulha_amanhecer.this, "Usuário não autenticado.", Toast.LENGTH_SHORT).show();
        }
    }
}
